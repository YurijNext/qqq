$(document).ready(function() {
    const styles = `
        ...
        <span>Color Schemes <a class="bg--colored" href="#"></a></span> 
        <div class="content">
    ...
    $(".switcher__color").prepend(styles);
});

jQuery(document).ready(function() {
    let cookieName = 'color-7';
    let defaultLayout = 'color-7';


    function changeLayout(layout) {
        jQuery.cookie(cookieName, layout);
        jQuery('head link[id=skins]').attr('href', 'css/colors/' + layout + '.css');
        jQuery('head link[id=skins]').attr('data-color', layout);
    ...
    if(!(jQuery('head link[id=skins]').attr('href')))
        jQuery('head link[id=skins]').attr('href', 'css/colors/' + defaultLayout + '.css');
    	jQuery('head link[id=skins]').attr('data-color', defaultLayout);
